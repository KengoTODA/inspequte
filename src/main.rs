...
fn expand_rule_args(args: &[String]) -> Result<Option<BTreeSet<String>>> {
    ...
    let base_dir = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    for arg in args {
        collect_rules_from_arg(arg, &base_dir, &mut stack, &mut rules)?;
    }
    if rules.is_empty() {
...
fn collect_rules_from_arg(
    ...
    for line in content.lines() {
        ...
        collect_rules_from_arg(line, file_dir, stack, rules)?;
    }
    stack.pop();
    Ok(())
}
...
mod tests {
    ...
    fn expand_rule_args_supports_at_file() {
        ...
        fs::write(
            &rules_file,
            "# selected rules\nSYSTEM_EXIT\nTHREAD_RUN_DIRECT_CALL,RETURN_IN_FINALLY\n",
        )
