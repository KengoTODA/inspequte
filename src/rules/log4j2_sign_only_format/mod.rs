...
/// Rule that flags Log4j2 format strings that are only placeholders.
#[derive(Default)]
pub(crate) struct Log4j2SignOnlyFormatRule;

crate::register_rule!(Log4j2SignOnlyFormatRule);

impl Rule for Log4j2SignOnlyFormatRule {
                        ...
                        if let Some(format_index) = format_index(&param_types) {
                                ...
                                args.get(format_index).cloned()
                            {
                                if is_placeholder_only(&text) {
                                    let message =
                                        result_message("Log4j2 format string should include text");
...
mod tests {
    ...
    fn analyze_sources(sources: Vec<SourceFile>) -> Vec<String> {
        ...
        output
            .results
            .iter()
            .filter(|result| result.rule_id.as_deref() == Some("LOG4J2_SIGN_ONLY_FORMAT"))
...
"#
    ...
    #[test]
    fn log4j2_sign_only_format_reports_placeholders_only() {
...
"#,
    ...
    #[test]
    fn log4j2_sign_only_format_allows_text() {
        let sources = log4j2_sources(
            r#"
package com.example;
import org.apache.logging.log4j.Logger;
...
public class ClassA {
    ...
    public void methodOne(String varThree) {
        ...
        fieldA.info("{} value", varThree);
        fieldA.debug(fieldB, "value {}", varThree);
    }
}
"#,
        );
