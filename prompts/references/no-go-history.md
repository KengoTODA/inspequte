...
Record failures without treating every No-Go as a rejected design.
...
- `disposition`: `design_rejected`, `implementation_failed`, `evidence_failed`, or `implemented`. Only `design_rejected` excludes a direction; mixed failures retain details in the reason.
